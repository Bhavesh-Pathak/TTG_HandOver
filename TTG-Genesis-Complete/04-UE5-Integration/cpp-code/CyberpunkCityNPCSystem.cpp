#include "CyberpunkCityNPCSystem.h"
...
ACyberpunkCityNPC::ACyberpunkCityNPC()
{
    ...
    // Bind overlap events
    InteractionSphere->OnComponentBeginOverlap.AddDynamic(this, &ACyberpunkCityNPC::OnInteractionSphereBeginOverlap);
    InteractionSphere->OnComponentEndOverlap.AddDynamic(this, &ACyberpunkCityNPC::OnInteractionSphereEndOverlap);
}

void ACyberpunkCityNPC::BeginPlay()
{
    Super::BeginPlay();
...
void ACyberpunkCityNPC::Tick(float DeltaTime)
{
...
void ACyberpunkCityNPC::InitializeFromData(const FCyberpunkCityNPCData& Data)
{
    NPCData = Data;
...
FString ACyberpunkCityNPC::GetCurrentDialogue()
{
...
FString ACyberpunkCityNPC::GetNextDialogue()
{
    if (HasMoreDialogue())
    {
...
bool ACyberpunkCityNPC::HasMoreDialogue() const
{
    return CurrentDialogueIndex < NPCData.Dialogue.Num() - 1;
}

void ACyberpunkCityNPC::ResetDialogue()
{
    CurrentDialogueIndex = 0;
}

void ACyberpunkCityNPC::StartInteraction(AActor* InteractingActor)
{
    if (bCanInteract)
    {
...
void ACyberpunkCityNPC::EndInteraction()
{
    OnInteractionEnded();
    ResetDialogue();
}

void ACyberpunkCityNPC::OnInteractionSphereBeginOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
...
void ACyberpunkCityNPC::OnInteractionSphereEndOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex)
{
...
void ACyberpunkCityNPC::UpdateBehavior()
{
...
void ACyberpunkCityNPC::HandlePatrolBehavior()
{
    // Basic patrol logic - can be expanded in Blueprint
}

void ACyberpunkCityNPC::HandleAggressiveBehavior()
{
    // Basic aggressive behavior - can be expanded in Blueprint
}

// NPC Manager Implementation
ACyberpunkCityNPCManager::ACyberpunkCityNPCManager()
{
    PrimaryActorTick.bCanEverTick = false;
}

void ACyberpunkCityNPCManager::BeginPlay()
{
    Super::BeginPlay();
...
void ACyberpunkCityNPCManager::LoadNPCsFromJSON()
{
...
ACyberpunkCityNPC* ACyberpunkCityNPCManager::SpawnNPC(const FString& NPCID, const FVector& Location, const FRotator& Rotation)
{
    if (!NPCClass)
    {
    ...
    // Find NPC data
    FCyberpunkCityNPCData* NPCData = NPCDatabase.FindByPredicate([&NPCID](const FCyberpunkCityNPCData& Data)
    {
    ...
    // Spawn NPC
    ACyberpunkCityNPC* SpawnedNPC = GetWorld()->SpawnActor<ACyberpunkCityNPC>(NPCClass, Location, Rotation);
    if (SpawnedNPC)
    {
...
ACyberpunkCityNPC* ACyberpunkCityNPCManager::FindNPCByID(const FString& NPCID)
{
    for (ACyberpunkCityNPC* NPC : SpawnedNPCs)
    {
...
TArray<ACyberpunkCityNPC*> ACyberpunkCityNPCManager::GetNPCsByType(ENPCType NPCType)
{
    TArray<ACyberpunkCityNPC*> FilteredNPCs;
...
void ACyberpunkCityNPCManager::SpawnAllNPCs()
{
